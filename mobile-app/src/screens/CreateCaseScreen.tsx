  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const caseData = {
        ...
        isOpenCase: !providerId,
        assignmentType: providerId ? 'specific' : 'open',
        chatSource: providerId ? 'inquiry' : 'direct',
      ...
      if (result.success) {
        const successMsg = providerId
          ? `Заявката е изпратена към ${providerName || 'специалиста'}! Той може да приеме, откаже или предложи друг бюджет.`
    ...
    >
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{providerId ? `Запитване към ${providerName || 'специалист'}` : 'Нова заявка'}</Text>
          <Text style={styles.subtitle}>{providerId ? 'Опишете от какво имате нужда и изпратете запитване' : 'Опишете от какво имате нужда'}</Text>
        </View>
