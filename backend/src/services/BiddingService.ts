...
export class BiddingService {
   ...
   */
  async placeBid(caseId: string, providerId: string): Promise<BidPlacementResult> {
    const client = await this.pool.connect();
    ...
    try {
      ...
      const caseBudget = parseFloat(caseData.budget);
      console.log('Calculating points for budget:', caseBudget, 'with limits:', tierLimits);
      const requiredPoints = this.pointsService.calculatePointsCost(caseBudget, tierLimits);
      console.log('Required points:', requiredPoints);

      // Check points balance (already fetched in caseQuery)
      if (caseData.points_balance < requiredPoints) {
        await client.query('ROLLBACK');
        return {
          success: false,
          message: `Insufficient points. Required: ${requiredPoints}, Available: ${caseData.points_balance}`
        };
      }
      ...
      // Create the bid (points not deducted yet, only reserved)
      const bidResult = await client.query(`
        ...
        RETURNING id
      `, [caseId, providerId, requiredPoints, bidOrder]);

      const bidId = bidResult.rows[0].id;

      // Temporarily deduct points (will be finalized when customer selects winner)
      ...
      `, [
        ...
        requiredPoints,
        newBalance,
        `Bid placed on case (${requiredPoints} points reserved)`,
        caseId
      ]);
      ...
      return {
        ...
        message: `Bid placed successfully. You are bidder #${bidOrder}`
      };
  ...
  async selectWinningBid(caseId: string, winningBidId: string, customerId: string): Promise<BidSelectionResult> {
    ...
    try {
      ...
      // Update winning bid status (points already deducted, just mark as won)
      await client.query(`
        UPDATE sp_case_bids
        SET bid_status = 'won',
      ...
      // Process losing bids (refund 80%, keep 20%)
      for (const bid of bids) {
        if (bid.id !== winningBidId) {
          const refundAmount = Math.floor(bid.points_bid * 0.8); // Refund 80%
          const deductedAmount = bid.points_bid - refundAmount; // Keep 20%
          ...
          const balanceResult = await client.query(`
            ...
            WHERE id = $3
            RETURNING points_balance
          `, [refundAmount, refundAmount, bid.provider_id]);
          ...
          `, [
            ...
            refundAmount,
            newBalance,
            `Bid refund (80%) for case - Lost bid`,
            caseId
          ]);
